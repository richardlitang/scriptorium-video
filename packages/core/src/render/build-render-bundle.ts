...
async function assertRenderableVisuals(projectDir: string, manifest: AssetManifest, timeline: Timeline): Promise<void> {
  const assetsById = new Map(manifest.assets.map((asset) => [asset.id, asset]));
  for (const segment of timeline.segments) {
    const visualAssetId = segment.mediaAssetIds[0];
    if (!visualAssetId) {
      throw new Error(`Timeline segment ${segment.beatId} has no visual asset. Generate images and sync before rendering.`);
    }
    const visualAsset = assetsById.get(visualAssetId);
    if (!isVisualAsset(visualAsset)) {
      throw new Error(`Timeline segment ${segment.beatId} references missing or non-visual asset ${visualAssetId}. Sync or regenerate images before rendering.`);
    }
    const absolutePath = path.resolve(projectDir, visualAsset.path);
    if (!absolutePath.startsWith(projectDir + path.sep)) {
      throw new Error(`Timeline segment ${segment.beatId} references visual asset outside the project directory.`);
    }
    try {
      await access(absolutePath);
    } catch {
      throw new Error(`Timeline segment ${segment.beatId} references missing visual file ${visualAsset.path}. Regenerate images before rendering.`);
    }
  }
}
