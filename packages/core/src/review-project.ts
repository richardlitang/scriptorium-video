...
export async function reviewProject(projectId: string, rootDir = process.cwd()): Promise<ReviewResult> {
  ...
  const [plan, manifest] = await Promise.all([
    readJsonFile(paths.videoPlan, VideoPlanSchema),
  ...
  const planHash = hashString(await readFile(paths.videoPlan, "utf8"));
  const timelineHash = await readFile(path.join(paths.projectDir, "timeline.json"), "utf8").then(hashString).catch(() => "");
  const runState = await readFile(path.join(paths.rootDir, ".studio-data", "run-state", `${projectId}.json`), "utf8")
    .then((raw) => JSON.parse(raw))
    .catch(() => ({}));
  if (runState.lastRenderPlanHash && (runState.lastRenderPlanHash !== planHash || runState.lastRenderTimelineHash !== timelineHash)) {
    issues.push(makeIssue({
      severity: "warning",
      scope: "render",
      code: "render_stale",
