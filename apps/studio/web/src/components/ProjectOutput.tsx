...
type OutputTab = "render" | "quality" | "timeline";

export function ProjectOutput({ projectId, timeline, captionCount, runState, needsRender }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex border-b border-[var(--color-border)] shrink-0">
        {(["render", "quality", "timeline"] as OutputTab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
          ...
          >
            {t === "render" ? "Render" : t === "quality" ? "Quality" : "Timeline"}
          </button>
        ))}
      ...
      <div className="flex-1 overflow-y-auto p-4">
        {tab === "render" && (
          <RenderTab draftRender={draftRender} isCurrent={isCurrent} />
        )}
        {tab === "quality" && (
          <QualityTab entries={qualityHistory ?? []} />
        )}
        {tab === "timeline" && (
